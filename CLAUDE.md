...
- **2026-06-29** — El identificador `code` ahora se persiste en Firestore (auto-generado, no editable). Al crear/editar un hallazgo, `upsertFinding` escribe `code = letra de categoría + número (order) padded` vía `formatFindingCode`; editar recalcula la letra si cambia la categoría, manteniendo el número (su `order`). `getAllSecurityFindings` prefiere el `code` guardado y cae al computado solo para docs sin backfill. `formatFindingCode` y el `code` resuelto se exponen en `NumberedSecurityFinding` (no en `SecurityFinding`). El `FindingModal` muestra el `code` de solo lectura en modo edición (en alta no existe aún). Backfill idempotente en `scripts/backfill-finding-codes.mjs` (replica el orden gap-free de `getAllSecurityFindings`). El mapa letra↔categoría del script debe mantenerse en sync con `SECURITY_CATEGORIES`.
- **2026-06-28** — Eliminado el campo `code` editable (C1/H3/etc.) de los hallazgos. El identificador visible ahora se compone de una letra de categoría + el número secuencial global, vía `formatFindingCode` en `security-catalog.ts` (I=iam, S=secrets, D=deps, P=platform, V=data, O=ops; ej. `P01`). Se quitó `code` del tipo `SecurityFinding`, de `FindingInput`, de la validación/persistencia (`sanitizeCatalog`), de la Server Action (`upsertSecurityFindingAction`, que ahora deriva el id solo del título) y del input del `FindingModal` (la primera fila es Severidad). El orden secundario en `getAllSecurityFindings` pasó de `code` a `title`.
